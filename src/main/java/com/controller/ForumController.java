    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            BeanUtils.copyProperties( forum , view );//把实体数据重构到view中
                ...
                //级联表
                BanzhuEntity banzhu = banzhuService.selectById(forum.getBanzhuId());
                if(banzhu != null){
                    BeanUtils.copyProperties( banzhu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setBanzhuId(banzhu.getId());
                }
                //级联表
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setBanzhuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("banzhu_id", forum.getBanzhuId())
...
//                            forumEntity.setBanzhuId(Integer.valueOf(data.get(0)));   //版主 要改的
        ...
        ForumEntity forum = forumService.selectById(id);
            if(forum !=null){
                ...
                //级联表
                    BanzhuEntity banzhu = banzhuService.selectById(forum.getBanzhuId());
                if(banzhu != null){
                    BeanUtils.copyProperties( banzhu , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setBanzhuId(banzhu.getId());
                }
                //级联表
                    YonghuEntity yonghu = yonghuService.selectById(forum.getYonghuId());
